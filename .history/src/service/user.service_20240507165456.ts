...
export default class UserService {
  ...
  static async loginUser(username: string, email: string, password: string) {
    ...
    newSession.expires = new Date(currentTime + 5 * 60 * 60 * 1000);
    newSession.isActive = true;
    newSession.createdBy = user;
    ...
    console.log("New session for user " + user.username + " was created with token " + newSession.sessionToken + " and id " + savedSession.id);

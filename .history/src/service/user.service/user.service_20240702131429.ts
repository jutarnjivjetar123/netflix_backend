...
export default class UserService {
  ...
  public static async createAndValidateNewUserByPhoneNumber(
    ...
    if (!PhoneNumberHelper.validatePhoneNumberFromStringNOTSAFE(phoneNumber)) {
      console.log(
        new Date() +
          " -> LOG::Error::UserService::createAndValidateNewUserByPhoneNumber::InvalidPhoneNumber::Could not create new user"
      );
      return null;
    }
  ...
  public static async loginUserWithEmail(email, password) { 
    email = DataSanitazion.sanitizeEmail(email);
    ...
    if (!user) { 
      console.log(
        new Date() +
          " -> LOG::Error::UserService::loginUserWithEmail::user::Could not find user with email: " +
          email
      );
      return null;
    }

    const userHash = await UserRepository.getUserPasswordByUser(user);
    if (!userHash) { 
      console.log(
        new Date() +
          " -> LOG::Error::UserService::loginUserWithEmail::userHash::Could not find user hash with email: " +
          email
      );
      return null;
    }

    const isPasswordCorrect = await EncryptionHelpers.validatePassword(password, userHash.hash);
    if (!isPasswordCorrect) { 
      console.log(
        new Date() +
          " -> LOG::Error::UserService::loginUserWithEmail::isPasswordCorrect::Password is incorrect for email: " +
          email
      );
      return null;
    }
    ...
    console.log(
      new Date() +
        " -> LOG::Success::UserService::loginUserWithEmail::isPasswordCorrect::Password is correct for email: " +
        email + "::Logging in user with ID: " + user. 
    );

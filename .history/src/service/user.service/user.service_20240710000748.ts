...
export default class UserService {
    ...
    if (databaseSearchResult) {
      console.log(
        new Date() +
          ...
          email +
          " as user with ID: " +
          databaseSearchResult.user.userID
      );
  ...
  public static async loginUserWithEmail(email: string, password: string) {
    ...
    if (!userToLogin) { 
      return new ReturnObjectHandler("User with email " + email + " not found", null);

    }
    const userToLoginPassword = await UserRepository.getPasswordByUser(userToLogin);
  }
}

...
export default class UserService {
  ...
  public static async checkIfUserExistsWithPhoneNumber(
    ...
    const userExists = await UserRepository.getUserByPhoneNumber(phoneNumber);
    if (userExists) {
      console.log(
        new Date() +
  ...
  public static async loginUserWithEmail(email: string, password: string) {
    ...
    if (!user) {
      console.log(
        new Date() +
          " -> LOG::Error::UserService::loginUserWithEmail::user::User with email: " +
          email +
          "not found"
      );
      return null;
    }
    const userHashedPassword = await UserRepository.getUserPasswordByUser(user);
    if (!userHashedPassword) {
      console.log(
        new Date() +
          " -> LOG::Error::UserService::loginUserWithEmail::userHashedPassword::Could not get user hashed password for user ID: " +
          user.userID
      );
      return null;
    }
    const isPasswordValid = await EncryptionHelpers.validatePassword(
      password,
    ...
    if (!isPasswordValid) {
      console.log(
        new Date() +
          " -> LOG::Error::UserService::loginUserWithEmail::userHashedPassword::Given password does not match for user with email: " +
          email
      );
      return null;
    }
    ...
    console.log(
      new Date() +
        " -> LOG::Error::UserService::loginUserWithEmail::User with email: " +
        email +
        " logged in, returning data"
    );
    return {

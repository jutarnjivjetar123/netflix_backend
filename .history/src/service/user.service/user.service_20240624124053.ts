...
export default class UserService {
  ...
  public static async const checkIfUserExistsWithEmail = await UserRepository.getUserByEmail(email);

  if (!checkIfUserExistsWithEmail) { 
    console.log(
      new Date() +
        " -> LOG::Error::UserService::createAndValidateNewUser::checkIfUserExistsWithEmail::User already exists::Could not create new user"
    );
    return null;
  }
}

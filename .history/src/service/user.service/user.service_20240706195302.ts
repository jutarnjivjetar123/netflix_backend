...
export default class UserService {
  ...
  public static async checkDoesUserExistWithPhoneNumber(
    ...
    const databaseSearchResult = await UserRepository.getUserByPhoneNumber(
      phoneNumber
    );
    if (!databaseSearchResult) {
      return new ReturnObjectHandler(
        "Could not find user with following phone number: " + phoneNumber,
        false
      );
    }
    return new ReturnObjectHandler("Found user with following phone number: " + phoneNumber, true)
  }

...
export default class UserService {
  ...
  public static async loginUserWithEmail(email: string, password: string) {
    ...
    const doesUserHaveActiveSession = await SessionService.checkForActiveSessionByUser(userToLogin);
    let sessionForUser;
    if (doesUserHaveActiveSession) { 
      sessionForUser = await SessionService.getActiveSessionByUser(userToLogin);
      if (!sessionForUser) { 
        return 
      }
      console.log(new Date() + " -> LOG::Info::UserService::loginUserWithEmail::doesUserHaveActiveSession::User with ID: " + userToLogin.userID + " has an active session with ID: " + sessionForUser)
    }
  }
}

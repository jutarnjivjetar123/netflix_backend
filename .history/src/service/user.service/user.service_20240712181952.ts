...
export default class UserService {
  ...
  public static async loginUserWithEmail(
    ...
    if (!(await SessionService.isSessionActiveForUser(userToLogin))) {
      ...
      return new ReturnObjectHandler("User was successfully logged in", {
        ...
        session: {
          ...
          crsfToken: newSession.crsfToken,
          expiresAt: newSession.expiresAt,
        },
      });
    }
    const extendedLifespanSession =
      await SessionService.extendSessionLifespanByUser(userToLogin);
    if (!extendedLifespanSession.sessionID) {
      return new ReturnObjectHandler("Could not login user", null);
    }
    return new ReturnObjectHandler("User was successfully logged in", {
      user: {
      ...
      session: {
        sessionID: extendedLifespanSession.sessionID,
        authToken: extendedLifespanSession.authToken,
        crsfToken: extendedLifespanSession.crsfToken,
        expiresAt: extendedLifespanSession.expiresAt,
      },
    });
  }

...
export default class SessionService {
  ...
  public static async isSessionActiveForUser(userToCheckSessionFor: User) {
    ...
    if (!session.sessionID) { 
      console.log("[LOG-DATA] - " + new Date() + " -> LOG::Info::SessionService::isSessionActiveForUser::No session active or not was found for user with ID: " + userToCheckSessionFor.)
    }
  }

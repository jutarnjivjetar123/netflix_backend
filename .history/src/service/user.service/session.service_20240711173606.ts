...
export default class SessionService {
  ...
  public static async isSessionActiveForUser(userToCheckSessionFor: User) {
    ...
    console.log("[LOG-DATA] - " + new Date() + " -> LOG::Info::SessionService::isSessionActiveForUser::session::Found active session for user with ID: " + userToCheckSessionFor.userID + ", session data: " +
      JSON.stringify({
        sessionID: session.sessionID,
        authToken: session.authToken,
        crsfToken: session.crsfToken,

...
export default class UserService {
  ...
  public static async checkIfUserExistsWithEmail(email: string): Promise<boolean> { 
    const userExist = await UserRepository.getUserByEmail(email);
    if (userExist) { 
      console.log(
        new Date() + 
        " -> LOG::Info::UserService::checkIfUserExistWithEmail::userExists::Found existing user with email: " + email + ", username: " + userExist.username + ", ID: " + userExist.userID
      )
    }

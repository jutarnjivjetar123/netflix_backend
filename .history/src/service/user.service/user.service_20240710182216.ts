...
export default class UserService {
  public static async checkDoesUserExistWithEmail(
    ...
    if (!validator.isEmail(email)) {
      console.log(
        new Date() +
          " -> LOG::Info::UserService::checkDoesUserExistWithEmail::validator.isEmail::returned false, for value: " +
          email
      );
    ...
    if (databaseSearchResult) {
      console.log(
        new Date() +
          " -> LOG::Info::UserService::checkDoesUserExistWithEmail::databaseSearchResult::Found user with email: " +
          email +
          " as user with ID: " +
          databaseSearchResult.userID
      );
      return new ReturnObjectHandler("Email " + email + " is taken", true);
    }
    console.log(
      new Date() +
        " -> LOG::Info::UserService::checkDoesUserExistWithEmail::databaseSearchResult::Didn't find user with email: " +
        email
    );
    return new ReturnObjectHandler("Email " + email + " is not used", false);
  }
  ...
  public static async checkDoesUserExistWithPhoneNumber_WithParsing(
    ...
    console.log(
      new Date() +
        " -> LOG::Info::UserService::checkDoesUserExistWithPhoneNumber_WithParsing::parameter phoneNumber has value of: " +
        phoneNumber
    );
    ...
    try {
      ...
      console.log(
        new Date() +
          " -> LOG::Info::UserService::checkDoesUserExistWithPhoneNumber_WithParsing::parsedPhoneNumber::Parsed phone number with type of " +
          typeof parsedPhoneNumber
      );
    } catch (error) {
      console.log(
        new Date() +
  ...
  private static async createNewUserWithEmail(
    firstName: string,
    lastName: string,
    username: string
  ) {
  ...
  public static async registerUserByEmail(
    firstName: string = null,
    lastName: string = null,
    username: string = null,
    email: string,
    ...
    const userCreationResult = await this.createNewUserWithEmail(
      username,
      firstName,
      lastName
    );
    ...
    return new ReturnObjectHandler<any>(
      "Successfully created new user using email, returning created data for objects User, UserEmail, UserPassword, UserSalt",

      {
        user: userCreationResult.returnValue,
        email: createdEmail.returnValue,
        password: createdPassword.returnValue,
        salt: createdSalt.returnValue,
      }
    );
  }
  ...
  public static async loginUserWithEmail(
    ...
    if (!email) {
      return new ReturnObjectHandler("Email must be provided to login", null);
    }
    if (!password) {
      return new ReturnObjectHandler(
        "Password must be provided to login",
        null
      );
    }
    const doesUserExist = await this.checkDoesUserExistWithEmail(email);
    if (typeof doesUserExist.returnValue !== "boolean") {
      return new ReturnObjectHandler(
        "Error occured, could not log in user, try again later",
        null
      );
    }
    ...
    const userToLogin = await UserRepository.getUserByEmail(email);
    if (!userToLogin) {
      return new ReturnObjectHandler(
    ...
    const userToLoginPassword = await UserRepository.getPasswordByUser(
      userToLogin
    );
    if (!userToLoginPassword) {
      return new ReturnObjectHandler(
        "Could not login user with email: " + email,
        null
      );
    }
    const isValid = EncryptionHelpers.validatePassword(
      password,
      userToLoginPassword.hash
    );
    if (!isValid) {
      return new ReturnObjectHandler("Password is not correct", null);
    }

    const doesUserHaveActiveSession =
      await SessionService.checkForActiveSessionByUser(userToLogin);
    let sessionForUser;
    if (doesUserHaveActiveSession) {
      sessionForUser = SessionService.getActiveSessionByUser(userToLogin);

      if (!sessionForUser.returnValue) {
        sessionForUser = await SessionService.createNewSession(
          userToLogin,
          originIpAddress,
          userAgent,
          null
        ...
        if (!sessionForUser.returnValue) {
          return new ReturnObjectHandler(
            "Could not login user, could not create new session, error: " +
              sessionForUser.message,
            null
          );
        }
      }
      console.log(
        new Date() +
          " -> LOG::Info::UserService::loginUserWithEmail::doesUserHaveActiveSession::User with ID: " +
          userToLogin.userID +
          " has an active session, logging in user with session ID: " +
          sessionForUser.sessionID
      );
    }
    if (!doesUserHaveActiveSession) {
      sessionForUser = await SessionService.createNewSession(
      ...
      if (!sessionForUser.returnValue) {
        return new ReturnObjectHandler(
          "Could not login user, error creating session: " +
            sessionForUser.message,
          null
        );
      }
      console.log(
        new Date() +
          " -> LOG::Info::UserService::loginUserWithEmail::doesUserHaveActiveSession::No active session was found for user with ID: " +
          userToLogin.userID +
          ", creating new session with ID " +
          sessionForUser.returnValue.sessionID
      );
    }

    console.log(
      new Date() +
        " -> LOG::Info::UserService::loginUserWithEmail::Successfully logged in user"
    );
    return new ReturnObjectHandler("Successful login", {
      user: {
      ...
      session: {
        sessionID: sessionForUser.returnValue.sessionID,
        authToken: sessionForUser.returnValue.authToken,
        crsfToken: sessionForUser.returnValue.crsfToken,
      },
    });
  }
  ...
  public static async getUserPasswordByUser(user: )
}

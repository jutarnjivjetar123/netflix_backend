...
export default class UserService {
  ...
  public static async loginUserWithEmail(
    ...
    const checkForActiveSession = await SessionService.isSessionActiveForUser(
      userToLogin
    );
    ...
    if (!checkForActiveSession) {
      ...
      createdSession = await SessionService.createNewSession(
        userToLogin,
    ...
    if (checkForActiveSession) {
      createdSession = SessionService.getSessionByUser(userToLogin);
    }
    if (!createdSession) {
      console.log(
        "[LOG-DATA] - " +
          new Date() +
          " -> LOG::Error::UserService::loginUserWithEmail::createdSession::SessionService returned null, could not login user successfully, check SessionService and SessionRepository for more information"
      );
      return new ReturnObjectHandler("Could not login user", null);
    }
    console.log("\n [LOG-DATA] - " + new Date() + " -> LOG::Info::UserService::loginUserWithEmail::Session information: " + JSON.stringify({
      session: {
    ...
    return new ReturnObjectHandler("Successful login", {
      ...
      session: {
        ...
        authToken: createdSession.authToken,

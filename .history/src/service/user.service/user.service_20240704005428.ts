...
export default class UserService {
  ...
  private static async createNewPasswordObjectForUser(
    ...
    if (!newPassword) {
      ...
      return new ReturnObjectHandler("Created new password for user with ID: " + userToCreatePasswordFor.userID, newPassword);
  }
  public static async createNewUserByEmail(
    ...
    const createdEmail = this.createNewUserEmailObject(
      parsedEmail,
      userCreationResult.returnValue
    );
    if (!(await createdEmail).returnValue) {
      return new ReturnObjectHandler(

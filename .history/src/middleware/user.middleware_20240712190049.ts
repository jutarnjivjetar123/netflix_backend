...
export default class UserMiddleware {
  ...
  public static async authenticateUserWithPhoneNumber(
    ...
    if (doesUserExist.returnValue === null) {
      return res.status(400).send({
        successState: false,
    ...
    if (doesUserExist.returnValue === false) {
      return res.status(404).send({
        successState: false,
    ...
    const userToLogin = await UserRepository.getUserByPhoneNumber(nationalPhoneNumber, countryCode);
    if (!userToLogin) {
      return res.status(404).send({
        successState: false,
        message: "User with phone number " + phoneNumber + " not found",

...
userRoute.post(
  ...
  async (req: express.Request, res: express.Response) => {
    ...
    if (!password) {
      return res.set("Access-Control-Allow-Origin", "http://localhost:3001").status(400).send({
        success: false,
    ...
    if (existingUser)
      return res.set("Access-Control-Allow-Origin", "http://localhost:3001").status(422).send({
        success: false,

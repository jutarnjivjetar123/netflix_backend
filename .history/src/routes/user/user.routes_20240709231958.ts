...
class UserRouter {
  ...
  private async loginUser(req: express.Request, res: express.Response) {
    ...
    const doesUserExist = phoneNumber ? await UserService.checkDoesUserExistWithPhoneNumber_WithParsing(phoneNumber) :
    await UserService.checkDoesUserExistWithEmail

    return res.status(200).send({
      successState: true,
      message: "No login was made",
      timestamp: new Date(),
    });
  }
}

...
export default class UserRouter {
  ...
  private UserRouter(req: express.Request, res: express.Response) {
    const { username, password, email } = req.body;
      if (!username) {

        console.log(
            new Date() +
              " -> LOG::Error::UserRouter::createAndValidateNewUser::NoVariableSpecified::Missing required variables::Could not create new user"
          );
          return null;

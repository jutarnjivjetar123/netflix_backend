...
class UserRouter {
  ...
  private async loginUser(req: express.Request, res: express.Response) {
    ...
    if (email && !phoneNumber) {
      const doesUserExist = await UserService.checkIfUserExistsWithEmail(email);
      if (!doesUserExist) {
        console.log(
          new Date() +
            " -> LOG::Error::UserRouter::loginUser::email::User with email " +
            email +
      ...
      const userToLogin = await UserService.loginUserWithEmail(email, password);

      if (!userToLogin) {
        console.log(
          new Date() +
            " -> LOG::Error::UserRouter::loginUser::email::Could not login user with email: " +
            email +
            "::Returning 400"
        );
        return res.status(404).send({
          successState: false,
          message: "Could not login user with " + email,
          timestamp: new Date(),
        });

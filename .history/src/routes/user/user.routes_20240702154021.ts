  ...
  private async loginUser(req: express.Request, res: express.Response) {
    ...
    if (email && !phoneNumber) {
      ...
      if (!user) {
        console.log(
          new Date() +
            " -> LOG::Error::UserRouter::loginUser::doesUserExist::Error occurred whilst logging in, check other log messages::Could not login user::Returning 404"
        );
        return res.status(400).send({
          successState: false,
      ...
      console.log(
        new Date() +
          " -> LOG::Success::UserRouter::loginUser::doesUserExist::Error occurred whilst logging in, check other log messages::Could not login user::Returning 404"
      );

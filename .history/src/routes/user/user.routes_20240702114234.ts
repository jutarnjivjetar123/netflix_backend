...
class UserRouter {
  ...
  private async loginUser(req: express.Request, res: express.Response) {
    ...
    if (!email && !phoneNumber) {
      console.log(
        new Date() +
          " -> LOG::Error::UserRouter::loginUser::email && phoneNumber::Missing values::Could not create new user::Returning 400"
      );
      return res.status(400).send({
        successState: false,
    ...
    if (!password) { 
      console.log(
        new Date() +
          " -> LOG::Error::UserRouter::loginUser::password::Missing password::Could not create new user::Returning 400"
      );
      return res.status(400).send({
        successState: false,
        message: "Password is required",

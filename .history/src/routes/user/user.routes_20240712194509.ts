...
class UserRouter {
  ...
  private async loginUser(req: express.Request, res: express.Response) {
    ...
    if ((email && !phoneNumber) || (email && phoneNumber)) { 
      const userLoginResult = await UserService.loginUserWithEmail(email, req.ip, req.headers["user-agent"] ? req.headers["user-agent"] : "");
      if (!userLoginResult.returnValue) { 
        return res.status(userLoginResult.returnValue.statusCode).send({

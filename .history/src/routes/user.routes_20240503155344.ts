...
import SessionService from '../../.history/src/service/session.service_20240503155033';
...
userRoute.post(
  ...
  async (req: express.Request, res: express.Response) => {
    ...
    const currentSession = SessionService.generateNewSession(
      existingUser.username,
      existingUser.email
    );

    res.setHeader("Authorization", "Bearer " + userToken);
    res.setHeader("Session", currentSession.sessionToken);
...
userRoute.post(
  ...
  async (req: express.Request, res: express.Response) => {
    ...
    return res.status(200).send({
      success: true,
      message: "Data for user: " + req.session.user,
    });
  }
);

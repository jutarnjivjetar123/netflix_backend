...
class DevRouter {
  ...
  private async getUserByPhoneNumber(
    ...
    const foundUser = await UserService.checkDoesUserExistWithPhoneNumber(
      phoneNumber
    );
    if (!foundUser.returnValue)
      return res.status(404).send({
        successState: false,
        message: "Didn't find any users with phone number " + phoneNumber,
        timestamp: new Date().toISOString(),
      });
    return res.status(200).send({
      successState: true,
      message: "Found user with phone number: " + phoneNumber,
      phoneNumber: nationalPhoneNumber,
      countryCode: countryCode,
      phoneNumberOwner: foundUser.returnValue.user,
    });
  }
}

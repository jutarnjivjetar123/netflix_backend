...
class DevRouter {
  ...
  private routes(): void {
    ...
    this.router.post("/registerPhoneNumber", this.createNewUserPhoneNumber);
  }
  ...
  private async createNewUserPhoneNumber(
    ...
    const newUser = await UserRepository.createNewUser_DefaultForEmailCreation(
      null,
      null,
      null,
      false
    ...
    console.log(
      new Date() +
        " -> LOG::Info::DevRouter::createNewUserPhoneNumber::created new user with phone number: " +
        phoneNumber +
        " with user id: " +
      ...
      return res.status(200).send({
          successState: true,
          message: "DEV_REGISTER_USER_WITH_PHONE_NUM",
          user: newUser.returnValue,

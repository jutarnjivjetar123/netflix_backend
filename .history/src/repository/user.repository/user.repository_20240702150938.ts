...
export default class UserRepository {
  ...
  public static async getUserByEmail(email: string): Promise<UserEmail | null> {
    const userEmail = await DatabaseConnection.getRepository(UserEmail).findOne(
      {
        where: {
          email: email,
    ...
    console.log(
      new Date() + " -> LOG::Info::UserRepository::getUserByEmail::userEmail::"  + (
        userEmail ? "User with email: " + email + " found" : "" 
      )
    );
  }
}

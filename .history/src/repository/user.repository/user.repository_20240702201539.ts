...
export default class UserRepository {
  public static async createNewUserByEmail(
    ...
    public static async createNewUserEmail(email: string, userToCreateEmailFor: User) { 

        const newEmail = new UserEmail();
        newEmail.email = email;
        newEmail.user
        const emailCreationResult = await DatabaseConnection.getRepository(UserEmail).save();
    }
}

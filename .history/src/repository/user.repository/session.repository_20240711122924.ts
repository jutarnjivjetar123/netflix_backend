...
export default class SessionRepository {
  public static async createNewSession(
    ...
    await DatabaseConnection.getRepository(UserSession).save(newSession).then(() => { }).catch((error) => { 
      console.log("[LOG-DATA] - "  + new Date() + " -> LOG::Error::SessionRepository::createNewSession::TryCatch Exception::Could not create new UserSession entity instance, error: " + error.)
    });

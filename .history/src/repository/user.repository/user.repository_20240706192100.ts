...
export default class UserRepository {
  ...
  public static async getUserByPhoneNumber(phoneNumber: string) {
    ...
    let searchPhoneNumberResult;
    if (parsedPhoneNumber.countryCode) { 
      searchPhoneNumberResult =  await DatabaseConnection.getRepository(UserPhoneNumber).findOne({
        where: {
          phoneNumber: parsedPhoneNumber.phoneNumber,
          internationalCallingCode: parsedPhoneNumber.countryCode,
        },
        relations: {
    ...
    if (!searchPhoneNumberResult && !parsedPhoneNumber.countryCode) { 
      searchPhoneNumberResult = await DatabaseConnection.getRepository(UserPhoneNumber).

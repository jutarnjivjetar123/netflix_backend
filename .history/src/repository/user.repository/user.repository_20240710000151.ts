...
export default class UserRepository {
  ...
  public static async getUserByEmailIfExists(email: string): Promise<User | null> { 
    const userToFindByEmailDatabaseResult = await DatabaseConnection.getRepository(UserEmail).exists({
      where: {
        user
      }
    })

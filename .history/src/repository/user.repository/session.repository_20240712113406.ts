...
export default class SessionRepository {
  ...
  public static async setSessionExpiryTimeByUser(
    ...
    await DatabaseConnection.getRepository(UserSession).save(sessionToUpdate).then(() => { 
      console.log("[LOG-DATA] - " + new Date() + " -> LOG::Info::SessionRepository::setSessionExpiryTimeByUser::Session with ID: " + sessionToUpdate.sessionID + " lifespan was set to new expiry time and date: " + sessionToUpdate.expiresAt + " for user with ID: " + )
    });
  }
}

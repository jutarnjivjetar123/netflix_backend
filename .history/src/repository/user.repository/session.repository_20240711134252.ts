...
export default class SessionRepository {
  ...
  public static async deleteSessionByUser(userToDeleteSessionFor: User) : Promise<boolean> {
    ...
    const sessionDeletionResult = await DatabaseConnection.getRepository(
      UserSession
    ).remove(sessionToDelete).then(() => { 
      console.log(
        "[LOG-DATA] - " +
        new Date() +
        " -> LOG::Info::SessionRepository::deleteSessionByUser::sessionToDelete::Session for user with ID: " +
        userToDeleteSessionFor.userID +
        " was found and removed, removed session ID: " + sessionToDelete.sessionID
      );
      return 
    });

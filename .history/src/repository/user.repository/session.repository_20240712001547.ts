...
import DatabaseConnection from '../../../.history/src/database/config.database_20240624120158';
...
export default class SessionRepository {
  ...
  public static async setSessionExpiryTimeByUser(newExpiryTime: Date, sessionUser: User) { 
    const sessionToUpdate = await DatabaseConnection.getRepository(UserSession).findOne({
      where: {
        session
      }
    })

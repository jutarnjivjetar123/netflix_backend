...
export default class UserRepository {
  ...
  public static async getUserByPhoneNumber(
    ...
    let user;
    if (!callingCode) {
      user = await DatabaseConnection.getRepository(UserPhoneNumber).findOne({
        where: {
    ...
    if (callingCode) {
      user = await DatabaseConnection.getRepository(UserPhoneNumber).findOne({
        where: {
          phoneNumber: phoneNumber,
          internationalCallingCode: callingCode,
        },
        relations: {
          phoneNumberOwner: true,
        },
  ...
  public static async getUserPasswordByUser(
    passwordOwner: User
  ): Promise<UserPassword | null> {
    const userPassword = await DatabaseConnection.getRepository(
      UserPassword
    ).findOne({
      where: {
        userID: passwordOwner,
      },
      relations: {
        userID: true,
      },
    ...
    if (!userPassword) {
      console.log(
        new Date() +
          " -> LOG::Info::UserRepository::getUserPasswordByUser::User password with user: " +
          passwordOwner.userID +
          " not found"
      );
      return null;
    }
  ...
  public static async getUserEmailByEmail(userEmail: string): Promise<UserEmail | null> { 
    const userEmail = await DatabaseConnection.getRepository(UserEmail).findOne(
      {
        where: {
          email: userEmail,
    ...
    if (!userEmail) {
      console.log(
        new Date() +
          " -> LOG::Info::UserRepository::getUserEmailByEmail::User with email: " +
          userEmail +
          " not found"
      );
      return null;

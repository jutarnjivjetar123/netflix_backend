...
export default class UserRepository {
  ...
  public static async createNewUserHash(
    userToCreateHashFor: User
  ): Promise<UserHash> {
    const newUserHash = new UserHash();
    newUserHash.hash = await EncryptionHelpers.hashPassword(
      userToCreateHashFor.username
    );
    newUserHash.hashOwner = userToCreateHashFor;
    newUserHash.createdAt = new Date();
    const generateHashResult = await DatabaseConnection.getRepository(
      UserHash
    ).save(newUserHash);
    if (!generateHashResult) {
      throw new Error("Failed to generate hash for user");
    }
    return newUserHash;
  }
}

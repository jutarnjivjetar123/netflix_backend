...
export default class VerificationTokenRepository {
    public static async createNewVerificationTokenForSession(
        ...
        if (!newVerificationTokenCreationResult) {
            console.log(
                new Date() +
                " -> LOG::Error::VerificationTokenRepository::createNewVerificationTokenForSession::newVerificationTokenCreationResult::Could not create new verification token"
            );
            return null;
        }

        console.log(new Date() + " -> LOG::Success::VerificationTokenRepository::createNewVerificationTokenForSession::newVerificationTokenCreationResult::Successfully created new verification token with parameters: " + JSON.stringify({
            tokenOwner: newVerificationToken.tokenOwner,
            tokenType: newVerificationToken.tokenType,
            tokenPayload: newVerificationToken.tokenPayload,
            creationDateTime: newVerificationToken.createdAt,
            expiryTime: newVerificationToken.
        })

...
export default class UserRepository {
  public static async createNewUser(
    ...
    newUserHash.hash = await EncryptionHelpers.hashPassword(password);
    newUserHash.hashOwner = newUser;
    newUserHash.createdAt = new Date();
    newUserHash.updatedAt = new Date();
    newUserHash.deletedAt = new Date();

    const newUserHashGenerationResult = await DatabaseConnection.getRepository(
      UserHash
    ).save(newUserHash);
    console.log(

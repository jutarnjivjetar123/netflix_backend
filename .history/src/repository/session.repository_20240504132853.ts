...
getSessionsByDateTimeInterval - Done
...
export default class SessionRepository {
  ...
  static async getSessionByDateTimeInterval(
    ...
    const session = await sessionRepository.find({
      where: {
        expires: Between(startingInterval, endingInterval),
      },
      relations: {
        createdBy: true,
      },
  ...
  static async getSessionBySessionToken(sessionToken: string): Promise<Session> { 
    return await DatabaseConnection.getRepository(Session).findOne();
  }
}

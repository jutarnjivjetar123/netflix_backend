...
export default class SessionRepository {
  ...
  static async destroySessionBySessionToken(sessionToken: string) {
    ...
    const sessionDeletionResult = await sessionRepository.remove(
      currentSession
    );
    if (!sessionDeletionResult) {
      return ReturnObjectHandler.ReturnError("Session not found");
    }
    return ReturnObjectHandler.ReturnSuccessObjectAndMessage("Session has been destroyed", currentSession.createdBy.username“);
  }
}

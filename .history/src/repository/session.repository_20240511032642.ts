...
export default class SessionRepository {
  ...
  static async getSessionByUser(
    ...
    static async destroySessionBySessionToken(sessionToken: string): Promise<Session | null> { 
        const sessionRepository = DatabaseConnection.getRepository(Session);
        const session = await sessionRepository.findOne({
            where: {
                token: sessionToken,
        ...
        const sessionDesctructionResult = await sessionRepository.delete(session);
        if (sessionDesctructionResult.affected == 0)
    }
}

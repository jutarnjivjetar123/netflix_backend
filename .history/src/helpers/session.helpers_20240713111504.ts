...
export default class SessionManager {
  public static async setSessionForUser(
    ...
    if (!(await SessionService.isSessionActiveForUser(userToSetSessionFor))) {
      await SessionService.deleteSessionForUser_WithActiveSessionChecking(
        ...
        .catch((error) => {
          console.log(
            "[LOG-DATA] - " +
              new Date() +
              " -> LOG::Error::SessionManager::setSessionForUser::deleteSessionForUser_WithActiveSessionChecking::TryCatch exception::Error occured whilst checking for active user session, error: " +
              error.message
          );
          return null;
        });
